...
mod tests {
    ...
    fn test_ml_kem_boringssl_decap() {
        ...
        let shared_secret =
            hex::decode("489dd1e9c2be4af3482bdb35bb26ce760e6e414da6ecbe489985748a825f1cd6")
                .unwrap();

        let key =
            PKey::private_key_from_raw_bytes_ex(&private_key, Variant::MlKem1024.as_str()).unwrap();
...
mod tests_boringssl {
    ...
    fn test_generate(variant: Variant) {
        ...
        // Decapsulate (encapsulation would need public key parsing which isn't implemented yet)
        // For now we can only test that the key was created successfully
        // TODO: Add encap/decap test once we implement public key operations
    }
    ...
    fn test_decapsulate(variant: Variant) {
        ...
        // 1. Generate key pair
        // 2. Encapsulate with public key -> (ciphertext, shared_secret_1)
        // 3. Decapsulate with private key -> shared_secret_2
        // 4. Assert shared_secret_1 == shared_secret_2
    }
}
